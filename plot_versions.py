...
plt.xlabel("Fix Commits Index (sorted by time)")
...
im = plt.imshow(matrix, aspect='auto', cmap=cmap, norm=norm, interpolation='none')  # overwrite with new cmap/norm
...
if isinstance(years, list) and len(years) == n_commits:
    ...
    for idx, year in enumerate(years):
        if year != last_year and year >= "2020":
            ...
            plt.text(idx, -1, str(year), ha='left', va='center', color='black', fontsize=12, rotation=0, zorder=5)
            last_year = year
            last_idx = idx
